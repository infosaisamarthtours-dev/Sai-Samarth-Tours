...
export function InternationalJourneys({ onSelectPackage, onOpenEnquiry }: Props) {
  // Specific international package IDs
  ...
  let internationalPackages = internationalIds
    .map(id => ALL_PACKAGES.find(p => p.id === id))
    .filter((p): p is Package => p !== undefined);

  // Fallback: fill up to 8 packages if internationalIds has missing entries
  if (internationalPackages.length < 8) {
    const existingIds = new Set(internationalPackages.map(p => p.id));
    for (const pkg of ALL_PACKAGES) {
      if (pkg.category === 'international' && !existingIds.has(pkg.id)) {
        internationalPackages.push(pkg);
  ...
  return (
    <section className="w-full py-12 md:py-20 bg-[#FBF9F5] border-b border-gray-200" id="international">
        ...
        <div className="text-center mb-8 sm:mb-12">
          ...
          <div className="hidden sm:flex flex-wrap justify-center items-center gap-2 sm:gap-4 mt-5">
            <span className="bg-white text-[#114088] text-xs font-bold px-3.5 py-1.5 rounded-full border border-blue-100 flex items-center gap-1.5 shadow-2xs">
              <span className="text-[#F59E0B]">✓</span> Complete Visa & Flight Assistance
            </span>
            <span className="bg-white text-[#114088] text-xs font-bold px-3.5 py-1.5 rounded-full border border-blue-100 flex items-center gap-1.5 shadow-2xs">
              <span className="text-[#F59E0B]">✓</span> Southeast Asia & European Highlights
            </span>
            <span className="bg-white text-[#114088] text-xs font-bold px-3.5 py-1.5 rounded-full border border-blue-100 flex items-center gap-1.5 shadow-2xs">
              <span className="text-[#F59E0B]">✓</span> Luxury Island Resorts & City Tours
            </span>
          </div>
            ...
            >
              ...
              <div className="relative h-40 w-full overflow-hidden shrink-0">
                ...
                <div className="absolute top-2.5 left-2.5 bg-[#114088]/90 text-amber-300 backdrop-blur-md px-2 py-0.5 rounded-full text-[9px] font-bold shadow-xs flex items-center gap-1">
                  <span>★ 4.9</span>
                  <span className="text-white/80 font-normal">| Global</span>
                </div>
